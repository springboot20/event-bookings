...
const EventForm = () => {
  ...
  async function onSubmit(values: InitialValues, { resetForm }: FormikHelpers<InitialValues>) {
    try {
      ...
      const categoryResponse = await addCategory({ name: values.category }).unwrap();
      ...
      formData.append('category', categoryResponse.data?._id);
      ...
      if (
        ...
        categoryResponse.statusCode.toString().startsWith('2')
      ) {
          ...
          <Form className='mt-4 w-full bg-white rounded-lg p-6 max-w-xl border'>
            ...
            <fieldset className='mt-2'>
              ...
              <div className='mt-2 relative'>
                <Field
                  ...
                  className={classNames(
                    'block w-full px-3 rounded border-0 py-3 text-gray-700 ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset text-sm outline-none',
                    errors.eventDate && touched.eventDate ? 'ring-red-600' : 'focus:ring-indigo-600'
                  )}
                />
              </div>
            </fieldset>
              ...
              <fieldset className='mt-2 flex-1'>
                ...
                <div className='relative mt-2'>
                  <Field
                    ...
                    id='category'
                    className={classNames(
                      'border-0 rounded w-full bg-white text-gray-800 first:text-gray-600 py-3 pl-3 pr-8 capitalize leading-tight focus:outline-none ring-1 ring-inset focus:ring-2 focus:ring-inset ring-gray-300  focus:ring-indigo-600',
            ...
            <div className='flex items-center gap-4 mt-4'>
              <button
                type='submit'
                disabled={creating}
                className=' font-semibold py-2.5 px-3 text-center uppercase tracking-wider disabled:ring-gray-200 disabled:pointer-events-none disabled:text-indigo-300 disabled:bg-[#FAFAFA] disabled:ring-1 text-base border-none ring-2 w-full ring-gray-200 rounded-md text-white bg-indigo-500'>
                {creating ? (
